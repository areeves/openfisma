...
class System {
	...
	public function systemExists($system_id = NULL) {
		...
		if ($system_id) {
			...
			// build our query
			$query = "SELECT `system_id` FROM `SYSTEMS` WHERE (`system_id` = '$system_id')";
	...
	public function getSystem($system_id = NULL) {
		...
		if ($system_id && $this->systemExists($system_id)) {
			...
			$query = "SELECT * FROM `SYSTEMS` WHERE (`system_id` = '$system_id')";
	...
	public function deleteSystem() {
		...
		if ($this->db) {
			...
			// define our query
			$query = "DELETE FROM `SYSTEMS` WHERE (`system_id` = '$this->system_id')";
	...
	public function getValidSystemIds($offset = 0, $limit = NULL) {
		...
		// create our query
		$query = "SELECT `system_id` FROM `SYSTEMS`";
