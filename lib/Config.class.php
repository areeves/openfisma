...
class Config {
  ...
  private function read_config() {
	...
	while ($line = fgets($FILE)) {
	  ...
	  if (!preg_match('/^#|^$/', $line)) {
		...
		if (preg_match('/APP_CONF/', $split[0])) { $this->APP_CONF = $split[1]; }
		if (preg_match('/APP_LIB/',  $split[0])) { $this->APP_LIB  = $split[1]; }
		if (preg_match('/APP_LOG/',  $split[0])) { $this->APP_LOG  = $split[1]; }
		if (preg_match('/APP_WWW/',  $split[0])) { $this->APP_WWW  = $split[1]; }
		if (preg_match('/APP_IMG/',  $split[0])) { $this->APP_IMG  = $split[1]; }
        ...
        if (preg_match('/PAGE_INTERVAL/', $split[0])) { $this->PAGE_INTERVAL = $split[1]; }
  ...
  public function APP_CONF()            { return $this->APP_ROOT().$this->APP_CONF; }
  public function APP_LIB()             { return $this->APP_ROOT().$this->APP_LIB;  }
  public function APP_LOG()             { return $this->APP_ROOT().$this->APP_LOG;  }
  public function APP_WWW()             { return $this->APP_ROOT().$this->APP_WWW;  }
  public function APP_IMG()             { return $this->APP_ROOT().$this->APP_IMG;  }
  ...
  public function PAGE_INTERVAL()       { return $this->PAGE_INTERVAL;       }
...
// debugging
$_ENV['OVMS_ROOT'] = 'c:/Program Files/xampp/htdocs/ovms/trunk';
