...
class Finding extends BaseFinding implements Fisma_Zend_Acl_OrganizationDependency
{
    ...
    public function getLongStatus($status = null)
    {
        ...
        switch ($status) {
            case 'NEW':
                return "{$this->status}: Awaiting Mitigation Strategy";
            case 'DRAFT':
                return "{$this->status}: Awaiting Mitigation Strategy Submission";
            case 'EN':
                return "{$this->status}: Awaiting Evidence Package Submission";
            case 'CLOSED':
                return "{$this->status}: Finding Officially Closed";
            default:
                return "{$this->status}: Awaiting {$activeEvaluation->name}";
        }
    }
}
