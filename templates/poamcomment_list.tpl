...
<table border='0' cellpadding='2' cellspacing='0'>
	...
	<tr>
		...
		<th align='left'>CommentParent
			<input type='image' src='images/button_up.png'   name='sort_params' value='comment_parent ASC'>
			<input type='image' src='images/button_down.png' name='sort_params' value='comment_parent DESC'>
		</th>

		<th align='left'>CommentDate
			<input type='image' src='images/button_up.png'   name='sort_params' value='comment_date ASC'>
			<input type='image' src='images/button_down.png' name='sort_params' value='comment_date DESC'>
		</th>
	...
	{* loop through the poamcomment list and populate with values *}
	{section name=poamcomment loop=$poamcomment_list}

	<tr bgcolor='{cycle values="#EEEEEE,#CCCCCC"}' >


		<td> {$poamcomment_list[poamcomment].poam_id} </td>
		<td> {$poamcomment_list[poamcomment].user_id} </td>
		<td> {$poamcomment_list[poamcomment].comment_parent} </td>
		<td> {$poamcomment_list[poamcomment].comment_date} </td>

		<form name='system_view' method='post' action='{$this_page}'>
		<td>
            <input type='hidden' name='form_target' value='poamcomment'>
			<input type='hidden' name='referrer'    value='{$this_page}'>
			<input type='hidden' name='comment_id'   value='{$poamcomment_list[poamcomment].comment_id}'>
