...
<table border='0' cellpadding='2' cellspacing='0'>
	...
	{* loop through the usersystemrole list and populate with values *}
	{section name=usersystemrole loop=$usersystemrole_list}

	<tr bgcolor='{cycle values="#EEEEEE,#CCCCCC"}' >


		<td> {$usersystemrole_list[usersystemrole].user_id} </td>
		<td> {$usersystemrole_list[usersystemrole].system_id} </td>
		<td> {$usersystemrole_list[usersystemrole].role_id} </td>

		<form name='system_view' method='post' action='{$this_page}'>
		<td>
            <input type='hidden' name='form_target' value='usersystemrole'>
			<input type='hidden' name='referrer'    value='{$this_page}'>
			<input type='hidden' name='usersystemrole_id'   value='{$usersystemrole_list[usersystemrole].usersystemrole_id}'>
