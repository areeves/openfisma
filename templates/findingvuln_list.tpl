...
<table border='0' cellpadding='2' cellspacing='0'>
	...
	<tr>
		...
		<th align='left'>VulnSeq
			<input type='image' src='images/button_up.png'   name='sort_params' value='vuln_seq ASC'>
			<input type='image' src='images/button_down.png' name='sort_params' value='vuln_seq DESC'>
		</th>
	...
	{* loop through the findingvuln list and populate with values *}
	{section name=findingvuln loop=$findingvuln_list}

	<tr bgcolor='{cycle values="#EEEEEE,#CCCCCC"}' >


		<td> {$findingvuln_list[findingvuln].vuln_type} </td>
		<td> {$findingvuln_list[findingvuln].vuln_seq} </td>
		<td> {$findingvuln_list[findingvuln].finding_id} </td>

		<form name='system_view' method='post' action='{$this_page}'>
		<td>
            <input type='hidden' name='form_target' value='findingvuln'>
			<input type='hidden' name='referrer'    value='{$this_page}'>
			<input type='hidden' name='findingvuln_id'   value='{$findingvuln_list[findingvuln].findingvuln_id}'>
