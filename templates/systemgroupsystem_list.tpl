...
<table border='0' cellpadding='2' cellspacing='0'>
	...
	{* loop through the systemgroupsystem list and populate with values *}
	{section name=systemgroupsystem loop=$systemgroupsystem_list}

	<tr bgcolor='{cycle values="#EEEEEE,#CCCCCC"}' >


		<td> {$systemgroupsystem_list[systemgroupsystem].sysgroup_id} </td>
		<td> {$systemgroupsystem_list[systemgroupsystem].system_id} </td>

		<form name='system_view' method='post' action='{$this_page}'>
		<td>
            <input type='hidden' name='form_target' value='systemgroupsystem'>
			<input type='hidden' name='referrer'    value='{$this_page}'>
			<input type='hidden' name='systemgroupsystem_id'   value='{$systemgroupsystem_list[systemgroupsystem].systemgroupsystem_id}'>
