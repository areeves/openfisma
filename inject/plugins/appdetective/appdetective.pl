...
# ---------------------------------------------------------------------------
# FILE     : apdetective.pl
# DESC     : Plugin to translate AppDetective scan XML-format reports
#             The plugin operates on the Vulnerability Details Report file.
...
require appdetective_translation;
...
# ---------------------------------------------------------------------------
	...
	#
        ...
        my ($std_string_arr_ref, $err_str_arr_ref) = appdetective_translation::translate($raw_data_string);

        if (scalar(@$err_str_arr_ref) > 0) {
          #
...
$inject_utils::this_name = "appdetective"; # TODO: put plugin name here (lowercase, no spaces)
