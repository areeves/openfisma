...
# ---------------------------------------------------------------------------
  ...
  if($success) {
    ...
    inject_utils::log_write(3, "unable to find suppressable vulnerability for asset $asset_id");
    }
...
# ---------------------------------------------------------------------------
  ...
  my ($success, @results) = inject_utils::db_exec($db, $sql);
  if(!$success) {
    inject_utils::log_write(2, "unable to find suppress finding $finding_id");
    }
} # -------------------------------------------------------------------------
