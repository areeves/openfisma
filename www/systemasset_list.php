...
case 'V': header("Location: ".$_CONFIG->APP_URL()."systemasset_view.php");   break;
case 'U': header("Location: ".$_CONFIG->APP_URL()."systemasset_update.php"); break;	
case 'D': header("Location: ".$_CONFIG->APP_URL()."systemasset_delete.php"); break;
...
// create the systemasset list for the listing
$systemassets = new SystemAssetList($_DB);

// select desired list parameters (column headers)
$systemassets->getAssetId();
$systemassets->getSystemId();
$systemassets->getSystemIsOwner();

// retrieve unique items in each column for filter options
$uniques = $systemassets->getUniques();
...
if ($_POST['form_action'] != 'reset') {
  ...
  // add filters to list
  if (isset($_POST['system_id'])           && ($_POST['system_id']          != '-- any --')) { $systemassets->filterSystemId($_POST['system_id'], $_POST['system_id_bool']);           }
  if (isset($_POST['asset_id'])            && ($_POST['asset_id']           != '-- any --')) { $systemassets->filterAssetId($_POST['asset_id'], $_POST['asset_id_bool']);            }
  
  // propagate sort parameters
  if (isset($_POST['sort_params'])) { $systemassets->setOrder($_POST['sort_params']); }
...
// reset the filters
  ...
  $_POST['asset_id_bool']       = NULL;

}

// retrieve the overall list size (for the pager)
$_PAGER->setListSize($systemassets->getListSize());
...
$systemasset_list = $systemassets->getList($_PAGER->getPageOffset(), $_PAGER->getPageSize());

// encrypt the ids
for ($row = 0; $row < count($systemasset_list); $row++) {

  $systemasset_list [$row]['systemasset_id'] = $_E->encrypt(serialize($systemasset_list [$row]));
...
// header
$_TEMPLATE->assign('this_page',  'systemasset_list.php');
$_TEMPLATE->assign('this_title', 'SYSTEMASSET > list');
...
$_TEMPLATE->assign('asset_ids',            $uniques['asset_id']);
...
$_TEMPLATE->assign('asset_id_bool',       $_POST['asset_id_bool']);
...
// systemasset_list
$_TEMPLATE->assign('systemasset_list', $systemasset_list);
...
// display the content
$_TEMPLATE->display('systemasset_filter.tpl');
$_TEMPLATE->display('pager.tpl');
$_TEMPLATE->display('systemasset_list.tpl');
