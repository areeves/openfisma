...
require_once('../lib/UsergroupList.class.php');
...
case 'V': header("Location: ".$_CONFIG->APP_URL()."usergroup_view.php");   break;
case 'U': header("Location: ".$_CONFIG->APP_URL()."usergroup_update.php"); break;	
case 'D': header("Location: ".$_CONFIG->APP_URL()."usergroup_delete.php"); break;
...
// create the usergroup list for the listing
$usergroups = new UsergroupList($_DB);

// select desired list parameters (column headers)
$usergroups->getUserGroupId();
$usergroups->getUserId();
$usergroups->getSysgroupId();

// retrieve unique items in each column for filter options
$uniques = $usergroups->getUniques();

require_once('../lib/UserList.class.php');
$users = new UserList($_DB);
$users->getUserId(TRUE);
$users->getUserName();
$users->filterUserId($uniques['user_id']);
$users->setOrder('user_name ASC');
$uniques['user_id'] = $users->getKeyList();
...
if ($_POST['form_action'] != 'reset') {
  ...
  // add filters to list
  if (isset($_POST['user_id'])               && ($_POST['user_id']              != '-- any --')) { $usergroups->filterUserId($_POST['user_id'], $_POST['user_id_bool']);               }
  if (isset($_POST['sysgroup_id'])           && ($_POST['sysgroup_id']          != '-- any --')) { $usergroups->filterSysgroupId($_POST['sysgroup_id'], $_POST['sysgroup_id_bool']);           }
  
  // propagate sort parameters
  if (isset($_POST['sort_params'])) { $usergroups->setOrder($_POST['sort_params']); }
...
$_PAGER->setListSize($usergroups->getListSize());
...
$usergroup_list = $usergroups->getList($_PAGER->getPageOffset(), $_PAGER->getPageSize());

// encrypt the ids
for ($row = 0; $row < count($usergroup_list); $row++) {

  $usergroup_list [$row]['user_group_id'] = $_E->encrypt($usergroup_list [$row]['user_group_id']);
...
// header
$_TEMPLATE->assign('this_page',  'usergroup_list.php');
$_TEMPLATE->assign('this_title', 'USERGROUP > list');
...
// usergroup_list
$_TEMPLATE->assign('usergroup_list', $usergroup_list);
...
// display the content
$_TEMPLATE->display('usergroup_filter.tpl');
$_TEMPLATE->display('pager.tpl');
$_TEMPLATE->display('usergroup_list.tpl');
