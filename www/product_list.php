...
case 'V': header("Location: ".$_CONFIG->APP_URL()."product_view.php");   break;
case 'U': header("Location: ".$_CONFIG->APP_URL()."product_update.php"); break;	
case 'D': header("Location: ".$_CONFIG->APP_URL()."product_delete.php"); break;
...
// create the product list for the listing
$products = new ProductList($_DB);

// select desired list parameters (column headers)
$products->getProdId();
$products->getProdName();
$products->getProdVersion();
$products->getProdDesc();

// retrieve unique items in each column for filter options
$uniques = $products->getUniques();
...
if ($_POST['form_action'] != 'reset') {
  ...
  // add filters to list
  if (isset($_POST['prod_name'])              && ($_POST['prod_name']             != '-- any --')) { $products->filterProdName($_POST['prod_name'], $_POST['prod_name_bool']);              }
  if (isset($_POST['prod_version'])           && ($_POST['prod_version']          != '-- any --')) { $products->filterProdVersion($_POST['prod_version'], $_POST['prod_version_bool']);           }
  if (isset($_POST['prod_desc'])              && ($_POST['prod_desc']             != '-- any --')) { $products->filterProdDesc($_POST['prod_desc'], $_POST['prod_desc_bool']);              }
  
  // propagate sort parameters
  if (isset($_POST['sort_params'])) { $products->setOrder($_POST['sort_params']); }
...
// reset the filters
  ...
  $_POST['prod_name']              = NULL;
  $_POST['prod_version']           = NULL;
  $_POST['prod_desc']              = NULL;

  $_POST['prod_name_bool']         = NULL;
  $_POST['prod_version_bool']      = NULL;
  $_POST['prod_desc_bool']         = NULL;

}

// retrieve the overall list size (for the pager)
$_PAGER->setListSize($products->getListSize());
...
$product_list = $products->getList($_PAGER->getPageOffset(), $_PAGER->getPageSize());

// encrypt the ids
for ($row = 0; $row < count($product_list); $row++) {

  $product_list [$row]['prod_id'] = $_E->encrypt($product_list [$row]['prod_id']);
...
// header
$_TEMPLATE->assign('this_page',  'product_list.php');
$_TEMPLATE->assign('this_title', 'PRODUCT > list');
...
$_TEMPLATE->assign('prod_names',              $uniques['prod_name']);
$_TEMPLATE->assign('prod_versions',           $uniques['prod_version']);
$_TEMPLATE->assign('prod_descs',              $uniques['prod_desc']);

$_TEMPLATE->assign('prod_name',              $_POST['prod_name']);
$_TEMPLATE->assign('prod_version',           $_POST['prod_version']);
$_TEMPLATE->assign('prod_desc',              $_POST['prod_desc']);

$_TEMPLATE->assign('prod_name_bool',         $_POST['prod_name_bool']);
$_TEMPLATE->assign('prod_version_bool',      $_POST['prod_version_bool']);
$_TEMPLATE->assign('prod_desc_bool',         $_POST['prod_desc_bool']);
...
// product_list
$_TEMPLATE->assign('product_list', $product_list);
...
// display the content
$_TEMPLATE->display('product_filter.tpl');
$_TEMPLATE->display('pager.tpl');
$_TEMPLATE->display('product_list.tpl');
