...
case 'V': header("Location: ".$_CONFIG->APP_URL()."asset_view.php");   break;
case 'U': header("Location: ".$_CONFIG->APP_URL()."asset_update.php"); break;	
case 'D': header("Location: ".$_CONFIG->APP_URL()."asset_delete.php"); break;
...
if ($_POST['form_action'] != 'reset') {
  ...
  // add filters to list
  if (isset($_POST['asset_name'])                   && ($_POST['asset_name']                  != '-- any --')) { $assets->filterAssetName($_POST['asset_name'], $_POST['asset_name_bool']);                   }
  if (isset($_POST['asset_date_created'])           && ($_POST['asset_date_created']          != '-- any --')) { $assets->filterAssetDateCreated($_POST['asset_date_created'], $_POST['asset_date_created_bool']);           }
  if (isset($_POST['asset_source'])                 && ($_POST['asset_source']                != '-- any --')) { $assets->filterAssetSource($_POST['asset_source'], $_POST['asset_source_bool']);                 }
  
  // propagate sort parameters
  if (isset($_POST['sort_params'])) { $assets->setOrder($_POST['sort_params']); }
...
// reset the filters
  ...
  $_POST['asset_name']                   = NULL;
  $_POST['asset_date_created']           = NULL;
  $_POST['asset_source']                 = NULL;

  $_POST['asset_name_bool']              = NULL;
  $_POST['asset_date_created_bool']      = NULL;
  $_POST['asset_source_bool']            = NULL;

}

// retrieve the overall list size (for the pager)
$_PAGER->setListSize($assets->getListSize());
...
$asset_list = $assets->getList($_PAGER->getPageOffset(), $_PAGER->getPageSize());

// encrypt the ids
for ($row = 0; $row < count($asset_list); $row++) {

  $asset_list [$row]['asset_id'] = $_E->encrypt($asset_list [$row]['asset_id']);
...
// header
$_TEMPLATE->assign('this_page',  'asset_list.php');
$_TEMPLATE->assign('this_title', 'ASSET > list');
...
$_TEMPLATE->assign('unique_names',                   $uniques['asset_name']);
$_TEMPLATE->assign('unique_date_createds',           $uniques['asset_date_created']);
$_TEMPLATE->assign('unique_sources',                 $uniques['asset_source']);

$_TEMPLATE->assign('selected_name',                   $_POST['asset_name']);
$_TEMPLATE->assign('selected_date_created',           $_POST['asset_date_created']);
$_TEMPLATE->assign('selected_source',                 $_POST['asset_source']);

$_TEMPLATE->assign('selected_name_bool',              $_POST['asset_name_bool']);
$_TEMPLATE->assign('selected_date_created_bool',      $_POST['asset_date_created_bool']);
$_TEMPLATE->assign('selected_source_bool',            $_POST['asset_source_bool']);
...
// asset_list
$_TEMPLATE->assign('asset_list', $asset_list);
...
// display the content
$_TEMPLATE->display('asset_filter.tpl');
$_TEMPLATE->display('pager.tpl');
$_TEMPLATE->display('asset_list.tpl');
