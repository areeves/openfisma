...
require_once('../lib/UserSystemRoleList.class.php');
...
case 'V': header("Location: ".$_CONFIG->APP_URL()."usersystemrole_view.php");   break;
case 'U': header("Location: ".$_CONFIG->APP_URL()."usersystemrole_update.php"); break;	
case 'D': header("Location: ".$_CONFIG->APP_URL()."usersystemrole_delete.php"); break;
...
// create the usersystemrole list for the listing
$usersystemroles = new UsersystemroleList($_DB);

// select desired list parameters (column headers)
$usersystemroles->getRoleId();
$usersystemroles->getSysgroupId();
$usersystemroles->getSystemId();
$usersystemroles->getUserId();

// retrieve unique items in each column for filter options
$uniques = $usersystemroles->getUniques();
...
if ($_POST['form_action'] != 'reset') {
  ...
  // add filters to list
  if (isset($_POST['user_id'])             && ($_POST['user_id']            != '-- any --')) { $usersystemroles->filterUserId($_POST['user_id'], $_POST['user_id_bool']);             }
  if (isset($_POST['system_id'])           && ($_POST['system_id']          != '-- any --')) { $usersystemroles->filterSystemId($_POST['system_id'], $_POST['system_id_bool']);           }
  if (isset($_POST['role_id'])             && ($_POST['role_id']            != '-- any --')) { $usersystemroles->filterRoleId($_POST['role_id'], $_POST['role_id_bool']);             }
  
  // propagate sort parameters
  if (isset($_POST['sort_params'])) { $usersystemroles->setOrder($_POST['sort_params']); }
...
$_PAGER->setListSize($usersystemroles->getListSize());
...
$usersystemrole_list = $usersystemroles->getList($_PAGER->getPageOffset(), $_PAGER->getPageSize());

// encrypt the ids
for ($row = 0; $row < count($usersystemrole_list); $row++) {

  $usersystemrole_list [$row]['usersystemrole_id'] = $_E->encrypt(serialize($usersystemrole_list [$row]));
...
// header
$_TEMPLATE->assign('this_page',  'usersystemrole_list.php');
$_TEMPLATE->assign('this_title', 'USERSYSTEMROLE > list');
...
// usersystemrole_list
$_TEMPLATE->assign('usersystemrole_list', $usersystemrole_list);
...
// display the content
$_TEMPLATE->display('usersystemrole_filter.tpl');
$_TEMPLATE->display('pager.tpl');
$_TEMPLATE->display('usersystemrole_list.tpl');
